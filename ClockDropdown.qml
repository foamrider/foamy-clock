// Adapted from Omarchy Quattro Ui/Dropdown.qml; see LICENSE-OMARCHY.
import QtQuick
...
Item {
  ...
  property bool spaceClockIcon: false
  ...
  component OptionLabel: Item {
    id: optionLabel
    property string text: ""
    property color color: root.foreground
    readonly property var parts: text.split("")
    readonly property string timeText: parts.length > 1 ? parts.slice(1).join("").trim() : ""
    readonly property bool splitClock: root.spaceClockIcon && timeText !== ""
    implicitHeight: plainText.implicitHeight
    clip: true

    Text {
      id: plainText
      visible: !optionLabel.splitClock
      anchors.fill: parent
    ...
    // Match the bar's optical icon bounds and six-pixel gaps, independent of font advance widths.
    Row {
      visible: optionLabel.splitClock
      height: parent.height
      spacing: Style.spaceReal(6)
      Text {
        width: Math.min(implicitWidth, Math.max(0, optionLabel.width - clockGlyph.width - timeLabel.width - parent.spacing * 2))
        height: parent.height
        text: optionLabel.parts[0].trim()
        textFormat: Text.PlainText
        color: optionLabel.color
        font.family: root.fontFamily
        font.pixelSize: Style.font.body
      ...
      Text {
        id: timeLabel
        width: Math.min(implicitWidth, Math.max(0, optionLabel.width - clockGlyph.width - parent.spacing * 2))
        height: parent.height
        text: optionLabel.timeText
        textFormat: Text.PlainText
        color: optionLabel.color
        font.family: root.fontFamily
        font.pixelSize: Style.font.body
